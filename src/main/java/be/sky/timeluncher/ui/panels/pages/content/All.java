...
import be.sky.timeluncher.game.AllTheInfos;
...
public class All extends ContentPanel{
    ...
    @Override
    public String getName() {return "All the mods";}

    @Override
    public String getStylesheetPath() {return "css/content/allthemods.css";}
    ...
    public void update(){
        ...
        IProgressCallback callback = new IProgressCallback() {
            ...
            public void step(Step step) {
                Platform.runLater(() -> {
                    stepTxt = All.StepInfo.valueOf(step.name()).getDetails();
            ...
            public void onFileDownloaded(Path path) {
                Platform.runLater(() -> {
                    ...
                    fileLabel.setText("..." + p.replace(Launcher.getInstanceall().getAllauncherDir().toFile().getAbsolutePath(), ""));
                });
            }
        };
        try {
            final VanillaVersion vanillaVersion = new VanillaVersion.VanillaVersionBuilder()
                    .withName(AllTheInfos.GAME_VERSION)
                    .build();

            //List<CurseFileInfo> curseMods = CurseFileInfo.getFilesFromJson(AllTheInfos.CURSE_MODS_LIST_URL);
                List<Mod> mods = Mod.getModsFromJson(AllTheInfos.MODS_LIST_URL);

            final AbstractForgeVersion forge = new ForgeVersionBuilder(AllTheInfos.FORGE_VERSION_TYPE)
                    .withForgeVersion(AllTheInfos.FORGE_VERSION)
            ...
            final FlowUpdater updater = new FlowUpdater.FlowUpdaterBuilder()
                    ...
                    .withLogger(Launcher.getInstanceall().getLogger())
                    .withProgressCallback(callback)
                    .build();

            updater.update(Launcher.getInstanceall().getAllauncherDir());
            this.startGame(updater.getVanillaVersion().getName());
        } catch (Exception e) {
            Launcher.getInstanceall().getLogger().printStackTrace(e);
    ...
    public void startGame(String gameVersion) {
        try {
            NoFramework noFramework = new NoFramework(
                    Launcher.getInstanceall().getAllauncherDir(),
                    Launcher.getInstanceall().getAuthInfos(),
            ...
            Process p = noFramework.launch(gameVersion, AllTheInfos.FORGE_VERSION.split("-")[1], NoFramework.ModLoader.FORGE);
